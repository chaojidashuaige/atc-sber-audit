...
@implementation ODMobileMBO_getActivities_pull_pq
...
static SUPEntityDelegate *g_ODMobileMBO_getActivities_pull_pq_delegate = nil;

+ (SUPEntityDelegate *) delegate
{
	@synchronized(self) {
		if (g_ODMobileMBO_getActivities_pull_pq_delegate == nil) {
			g_ODMobileMBO_getActivities_pull_pq_delegate = [[SUPEntityDelegate alloc] initWithName:@"ODMobileMBO_getActivities_pull_pq" clazz:[self class]
				metaData:[self metaData] dbDelegate:[ODMobileODMobileDB delegate] database:[ODMobileODMobileDB instance]];
		}
	}
	
	return [[g_ODMobileMBO_getActivities_pull_pq_delegate retain] autorelease];
}
...
static SUPEntityMetaDataRBS* ODMobileMBO_getActivities_pull_pq_META_DATA;

+ (SUPEntityMetaDataRBS*)metaData
{
    if (ODMobileMBO_getActivities_pull_pq_META_DATA == nil) {
		ODMobileMBO_getActivities_pull_pq_META_DATA = [[ODMobileMBO_getActivities_pull_pqMetaData alloc] init];
	}
	
	return ODMobileMBO_getActivities_pull_pq_META_DATA;
}
...
- (id) init
{
    if ((self = [super init]))
    {
        self.classMetaData = [ODMobileMBO_getActivities_pull_pq metaData];
        [self setEntityDelegate:(SUPEntityDelegate*)[ODMobileMBO_getActivities_pull_pq delegate]];
    }
    return self;    
}
...
+ (ODMobileMBO_getActivities_pull_pq*)find:(int64_t)id_
{
    ...
    return (ODMobileMBO_getActivities_pull_pq*)[(SUPEntityDelegate*)([[self class] delegate]) findEntityWithKeys:keys];
}
...
+ (ODMobileMBO_getActivities_pull_pq*)load:(int64_t)id_
{
    return (ODMobileMBO_getActivities_pull_pq*)[(SUPEntityDelegate*)([[self class] delegate]) load:[NSNumber numberWithLong:id_]];
}

+ (ODMobileMBO_getActivities_pull_pq*)getInstance
{
    ODMobileMBO_getActivities_pull_pq* me = [[ODMobileMBO_getActivities_pull_pq alloc] init];
    [me autorelease];
    return me;
}
...
{
   return [ODMobileLogRecordImpl findByEntity:@"MBO_getActivities_pull_pq":[self keyToString]];
}
...
+ (SUPObjectList*)findAll:(int32_t)skip take:(int32_t)take
{
	...
	return (SUPObjectList*)[[[self class] delegate] findWithSQL:sql withDataTypes:dts withValues:values withIDs:ids withSkip:skip withTake:take withClass:[ODMobileMBO_getActivities_pull_pq class]];
}
...
+ (ODMobileMBO_getActivities_pull_pq*)findSub:(NSString*)_username withEMPLOYEE_ID:(NSString*)eMPLOYEE_ID withOBJECT_NAME:(NSString*)oBJECT_NAME
{
	...
	SUPObjectList* res = (SUPObjectList*)[[[self class] delegate] findWithSQL:sql withDataTypes:dts withValues:values withIDs:ids withClass:[ODMobileMBO_getActivities_pull_pq class]];
	if(res && ([res size] > 0))
	{   
		ODMobileMBO_getActivities_pull_pq* cus = (ODMobileMBO_getActivities_pull_pq*)[res item:0];
	    return cus;
	}
	else
	    return nil;
