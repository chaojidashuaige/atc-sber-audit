...
@implementation ODMobileMBOBusinessProcessMetaData

+ (ODMobileMBOBusinessProcessMetaData*)getInstance
{
    return [[[ODMobileMBOBusinessProcessMetaData alloc] init] autorelease];
}
...
- (id)init
{
    if (self = [super init]) {
		self.id = 5;
		self.name = @"MBOBusinessProcess";
		self.klass = [ODMobileMBOBusinessProcess class];
		...
		SUPAttributeMetaDataRBS* a_BUSINESS_PROCESS_NAME = [SUPAttributeMetaDataRBS attributeMetaDataWith:
			...
			GeneratedScheme_NONE:
			NO:SUPPersonalizationType_None:NO];
		[a_BUSINESS_PROCESS_NAME setColumn:@"c"];
		...
		[attributes addThis:a_BUSINESS_PROCESS_KEY];
		[attributes addThis:a_BUSINESS_PROCESS_NAME];
		...
		SUPAttributeMap * attributeMap = [SUPAttributeMap getInstance];
 		...
 		SUPOperationMetaData* o_findAll_0 = [SUPOperationMetaData createOperationMetaData:1:(SUPString)@"findAll":[SUPDataType forName:@"MBOBusinessProcess*"]:true];
		[o_findAll_0 setIsStatic:YES];
		[o_findAll_0 setIsCreate:NO];
		[o_findAll_0 setIsUpdate:NO];
		[o_findAll_0 setIsDelete:NO]; 		
 		SUPOperationMetaData* o_findByPrimaryKey_1 = [SUPOperationMetaData createOperationMetaData:2:(SUPString)@"findByPrimaryKey":[SUPDataType forName:@"MBOBusinessProcess"]:true];
	  	{
			SUPObjectList *parameters_list = nil;
	 		SUPParameterMetaData* p_findByPrimaryKey_BUSINESS_PROCESS_ID = [SUPParameterMetaData createParameterMetaData:1:(SUPString)@"BUSINESS_PROCESS_ID":[SUPDataType forName:@"string"]];
 			parameters_list = [SUPObjectList listWithCapacity:1];
			[parameters_list addThis:p_findByPrimaryKey_BUSINESS_PROCESS_ID];
		...
		[o_save_0 setIsDelete:NO]; 		
 		SUPOperationMetaData* o_findWithQuery_1 = [SUPOperationMetaData createOperationMetaData:10:(SUPString)@"findWithQuery":[SUPDataType forName:@"MBOBusinessProcess*"]:true];
	  	{
			SUPObjectList *parameters_list = nil;
		...
		[i_findByPrimaryKeyIndex.attributes add:a_BUSINESS_PROCESS_ID];
