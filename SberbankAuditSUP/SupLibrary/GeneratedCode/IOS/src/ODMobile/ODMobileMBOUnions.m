...
@implementation ODMobileMBOUnions
...
static SUPEntityDelegate *g_ODMobileMBOUnions_delegate = nil;

+ (SUPEntityDelegate *) delegate
{
	@synchronized(self) {
		if (g_ODMobileMBOUnions_delegate == nil) {
			g_ODMobileMBOUnions_delegate = [[SUPEntityDelegate alloc] initWithName:@"ODMobileMBOUnions" clazz:[self class]
				metaData:[self metaData] dbDelegate:[ODMobileODMobileDB delegate] database:[ODMobileODMobileDB instance]];
		}
	}
	
	return [[g_ODMobileMBOUnions_delegate retain] autorelease];
}

static SUPEntityMetaDataRBS* ODMobileMBOUnions_META_DATA;

+ (SUPEntityMetaDataRBS*)metaData
{
    if (ODMobileMBOUnions_META_DATA == nil) {
		ODMobileMBOUnions_META_DATA = [[ODMobileMBOUnionsMetaData alloc] init];
	}
	
	return ODMobileMBOUnions_META_DATA;
}
...
- (id) init
{
    if ((self = [super init]))
    {
        self.classMetaData = [ODMobileMBOUnions metaData];
        [self setEntityDelegate:(SUPEntityDelegate*)[ODMobileMBOUnions delegate]];
    }
    return self;    
}
...
+ (ODMobileMBOUnions*)find:(int64_t)id_
{
    SUPObjectList *keys = [SUPObjectList getInstance];
    [keys add:[NSNumber numberWithLong:id_]];
    return (ODMobileMBOUnions*)[(SUPEntityDelegate*)([[self class] delegate]) findEntityWithKeys:keys];
}

+ (SUPObjectList*)findWithQuery:(SUPQuery*)query
{
    return (SUPObjectList*)[(SUPEntityDelegate*)([[self class] delegate])  findWithQuery:query:[ODMobileMBOUnions class]];
}
...
+ (ODMobileMBOUnions*)load:(int64_t)id_
{
    return (ODMobileMBOUnions*)[(SUPEntityDelegate*)([[self class] delegate]) load:[NSNumber numberWithLong:id_]];
}

+ (ODMobileMBOUnions*)getInstance
{
    ODMobileMBOUnions* me = [[ODMobileMBOUnions alloc] init];
    [me autorelease];
    return me;
}
...
- (ODMobileMBOUnions*)getDownloadState
{
    return (ODMobileMBOUnions*)[self i_getDownloadState];
}

- (ODMobileMBOUnions*) getOriginalState
{
    return (ODMobileMBOUnions*)[self i_getOriginalState];
}
...
{
   return [ODMobileLogRecordImpl findByEntity:@"MBOUnions":[self keyToString]];
}
...
+ (SUPObjectList*)findAll:(int32_t)skip take:(int32_t)take
{
	...
	return (SUPObjectList*)[[[self class] delegate] findWithSQL:sql withDataTypes:dts withValues:values withIDs:ids withSkip:skip withTake:take withClass:[ODMobileMBOUnions class]];
}



+ (ODMobileMBOUnions*)findByPrimaryKey:(NSString*)uNION_ID
{
	NSMutableString *sql = nil;
	NSMutableString *_selectSQL = nil;
	...
	SUPObjectList* res = (SUPObjectList*)[[[self class] delegate] findWithSQL:sql withDataTypes:dts withValues:values withIDs:ids withClass:[ODMobileMBOUnions class]];
	if(res && ([res size] > 0))
	{   
		ODMobileMBOUnions* cus = (ODMobileMBOUnions*)[res item:0];
	    return cus;
	}
	else
	    return nil;
 ...
 */
+ (ODMobileMBOUnionsSynchronizationParameters*)getSynchronizationParameters
{
    return (ODMobileMBOUnionsSynchronizationParameters*)[(SUPSyncParamEntityDelegate*)([[ODMobileODMobileDB delegate] getDelegate:@"MBOUnionsSynchronizationParameters"]) getSynchronizationParameters];
}
