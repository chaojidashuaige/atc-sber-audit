						...
						if ((((UIView*)[[slideViews subviews] objectAtIndex:0]).frame.origin.x+200) >= (self.view.frame.origin.x + ((UIView*)[[slideViews subviews] objectAtIndex:0]).frame.size.width)) {
							...
							if (viewControllerCount > 1) {
								for (int i = 1; i < viewControllerCount; i++) {
									viewXPosition = self.view.frame.size.width - [slideViews viewWithTag:i].frame.size.width;
									[[slideViews viewWithTag:i] removeFromSuperview];
                                    if([delegate respondsToSelector:@selector(viewControllerWillClose)])
                                    {
                                        [delegate performSelector:@selector(viewControllerWillClose)];
                                    }
									[viewControllersStack removeLastObject];
								}
								[[borderViews viewWithTag:3] setHidden:TRUE];
...
//		isContentSizeForMainViewSet = TRUE;
	...
	if([viewControllersStack count] > 1){
		NSUInteger indexOfViewController = [viewControllersStack
...
//add vertical line on slide if need

    if ([slideViews.subviews count] != 0) {
        
        if(isDraw)
        {
            UIImageView *verticalLineView = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"main_spacer_horizontal.png"]];
            [verticalLineView setFrame:CGRectMake(0, 0, 1, 1004)];
            [controller.view addSubview:verticalLineView];
            [verticalLineView release];
            
            UIImageView *verticalShadow = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"shadow.png"]];
            [verticalShadow setFrame:CGRectMake(-27, 0, 27, 1004)];
            [controller.view addSubview:verticalShadow];
            [verticalShadow release];
        }
	}
